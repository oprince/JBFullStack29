...
class LinkedList {
  ...
  search(node) {
    ...
    if (lastNode) {
      ...
      if (lastNode.match(node)) return lastNode;
    }
    return null;
  }
  searchAll(searchStr) {
    ...
    let lastNode = this.getFirst();
    while (lastNode) {
      if (lastNode.hasStr(searchStr)) foundNodes.push(lastNode);
      lastNode = lastNode.next;
    }
    return foundNodes;
  }
...
function addStudentPost(student) {
  $.ajax({
    type: "POST",
    url: "/student",
    data: student,
